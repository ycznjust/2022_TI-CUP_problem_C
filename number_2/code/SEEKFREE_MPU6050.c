 ...
 * @LastEditors: Ptisak
 * @LastEditTime: 2022-07-24 22:55:52
 * @Version: Do not edit
 */
/*********************************************************************************************************************
 * COPYRIGHT NOTICE
...
//  Sample usage:				调用该函数前，请先调用模拟IIC的初始化
//-------------------------------------------------------------------------------------------------------------------
void zf_mpu6050_init(void)
{
    simiic_init();
    delay_ms(100); //上电延时
    simiic_write_reg(MPU6050_DEV_ADDR, PWR_MGMT_1, 0x80);
    delay_ms(100);
    ...
    simiic_write_reg(MPU6050_DEV_ADDR, SMPLRT_DIV, 0x00);     // sample rate.  Fsample= 1Khz/(<this value>+1) = 1000Hz
    simiic_write_reg(MPU6050_DEV_ADDR, MPU6050_CONFIG, 0x03); //内部低通  acc:44hz	gyro:42hz
    simiic_write_reg(MPU6050_DEV_ADDR, GYRO_CONFIG, 0x18);    // gyro scale  ：+-2000°/s
    simiic_write_reg(MPU6050_DEV_ADDR, ACCEL_CONFIG, 0x10);   // Accel scale ：+-8g (65536/16=4096 LSB/g)
}
